class MediaCommandCenterCard extends HTMLElement {
  ...
  render() {
    ...
    const title = this.st(c.active_title, this.st(c.stremio_title, 'Nothing playing'));
    ...
    const art = this.attr(c.player, 'entity_picture') || this.attr(c.kodi, 'entity_picture') || '';
    ...
    this.shadowRoot.innerHTML = `
      ...
      <ha-card><div class="shell"><div class="wash"></div><div class="shade"></div><div class="grid">
        ...
        <section class="panel"><h3>Stremio Search</h3><div class="search"><input class="q" placeholder="Title" value="${this.esc(this.st(c.search_title,''))}"><select class="type"><option>Any</option><option>Movie</option><option>Series</option></select><input class="season" type="number" min="0" value="${this.esc(this.st(c.search_season,'0'))}"><button data-action="search">Search</button></div></section>
  ...
  bind() {
    ...
    this.shadowRoot.querySelectorAll('[data-action]').forEach(b => b.onclick = () => {
      ...
      if (b.dataset.action === 'search') this.script(this.config.search_script);
    });
    const q = this.shadowRoot.querySelector('.q'); if (q) q.onchange = e => this.setText(this.config.search_title, e.target.value);
